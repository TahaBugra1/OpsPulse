...
### 3a. Decide the prompt strategy (risk-tiered)
Inlining the full current content of every file the subagent will touch is
the safest default, but it isn't free — it costs your own context (you must
read the file first), the subagent's starting context, and duplicated read
effort neither of you strictly needed. Not every task justifies that cost.
Classify the task before building the prompt in step 4:

- **High-risk** (auth, the request/order/session state machine, anything
  authorization- or security-sensitive, WebSocket/realtime channels, a
  shared function multiple other call sites depend on, or anything the
  project's own docs flag for extra scrutiny): inline the exact current
  content of every file the subagent will modify, plus explicit call-outs of
  any non-obvious trap (a join/filter ordering that silently drops rows, an
  existing function that must stay byte-for-byte unchanged, a rounding or
  null-handling convention to match). This is the template shown in step 4.
- **Low-risk** (mechanical CRUD once a pattern exists, appending tests to an
  already-established test file, route/config wiring, anything the project's
  own usage guide already calls low-risk): give the subagent the absolute
  path(s) of the file(s) to read and modify instead of pasting their full
  content — tell it explicitly to `Read` them itself before editing. Still
  build and inline the task spec (2), CAVEMAN principles (2a), Definition of
  Done (2b), and Output Contract (2c) in full — those are cheap and are what
  actually prevents scope drift; only the large "here is the current file"
  block is what you're allowed to drop.

When unsure which tier applies, default to high-risk. Over-inlining wastes
tokens; under-inlining a genuinely risky task risks an implementation that
looks plausible and passes a shallow review by accident. This choice never
changes step 5 — independent verification (reading the result yourself,
running tests yourself) is mandatory in both tiers; the tier only affects how
much the subagent is handed up front, never whether you check its work after.
...
```
# High-risk tier — inline the full current content of every file to modify
Agent({
  ...
  prompt: (
    ...
    "CURRENT CONTENT of every file you will modify (read this, don't re-read "
    "the file yourself unless you need more context):\n<pasted verbatim>\n\n"
...
Agent({
  ...
  prompt: (
    ...
    "implementation files, no test files.\n\n"
    "PROJECT ROOT (absolute, write everything relative to this): <root>\n\n"
    "FILES TO MODIFY (read each one yourself before editing it): <absolute "
    "paths>\n\n"
    "TASK SPEC:\n<task spec from step 2>\n\n"
    "CAVEMAN PRINCIPLES:\n<step 2a>\n\n"
    "DEFINITION OF DONE:\n<step 2b>\n\n"
...
— **make this call once**, not speculatively; get the project root, file
list, and risk tier right first (steps 3/3a) instead of finding out via a
wasted run.
